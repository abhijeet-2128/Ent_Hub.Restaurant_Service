import { Controller, Post, Body, Param, Get, UseGuards, UnauthorizedException, OnModuleInit, Req, Query, Put, Res } from '@nestjs/common';
...
import { Restaurant } from './entity/restaurant.schemal';
...
//get restaurant by Id
  ...
  async createReservation(@Body() reservationRequest: ReservationRequestDTO, @Req() req, @Res() res) {
  ...
  async placeFoodDeliveryOrder(
    ...
    try {
      ...
      if (orderResult.message) {
        return { statusCode: 404, message: orderResult.message };
      }
      return { statusCode: 200, ...orderResult };
    } catch (error) {
      console.error(error);
